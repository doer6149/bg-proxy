...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers',
    'Content-Type, X-BG-KEY, X-BG-SECRET, X-BG-PASS, X-BG-PATH');
  ...
  const targetPath = req.headers['x-bg-path'];

  if (!apiKey || !apiSecret || !apiPass || !targetPath) {
    return res.status(400).json({ error: 'Missing credentials or path' });
  }
